      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-2">
        {projectList.map((project) => (
          <button
            ...
            className="rounded-3xl border border-slate-200 p-4 text-left transition hover:border-cyanAccent hover:bg-cyan-50/30 hover:shadow-soft"
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={cn("rounded-full border px-2.5 py-1 text-xs font-bold", priorityClass(project.opportunityLevel))}>
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Index global : donneurs d'ordre, MOA, AMO/MOE, intégrateurs, fournisseurs, sous-traitants, régulateurs et industriels fabricants.
      </p>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {actorList.map((actor) => (
          <button
            ...
            className="rounded-3xl border border-slate-200 p-4 text-left transition hover:border-orangeAccent hover:bg-orange-50/30 hover:shadow-soft"
          >
            <div className="flex items-start justify-between gap-3">
              <h3 className="font-bold text-night">{actor.name}</h3>

              <span className={cn("rounded-full border px-2.5 py-1 text-xs font-bold", priorityClass(actor.commercialPriority))}>
                {actor.commercialPriority}
              </span>
            </div>
...
function LoadPlanView({ onSelectProject }: { onSelectProject: (project: Project) => void }) {
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Vue qualitative construite uniquement à partir des informations présentes dans les documents. Les valeurs inconnues restent non précisées.
      </p>
              ...
              return (
                <tr key={item.id} className="align-top hover:bg-slate-50">
                  <td className="px-4 py-3 font-semibold text-night">
                    <button type="button" onClick={() => project && onSelectProject(project)} className="hover:text-cyanAccent">
                  ...
                  <td className="px-4 py-3">
                    <span className={cn("rounded-full border px-2.5 py-1 text-xs font-bold", priorityClass(item.opportunityLevel))}>
  ...
  return (
    <section className="space-y-5">
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
        <h2 className="flex items-center gap-2 text-xl font-bold text-night">
          <Database className="h-5 w-5 text-cyanAccent" /> Données intégrées
        </h2>

        <p className="mt-1 text-sm text-slate-600">
          Table de contrôle pour vérifier que les projets, acteurs, spécialités, comptes et relations sont embarqués dans l'application.
        </p>

        <div className="mt-4 grid gap-3 md:grid-cols-6">
          <div className="rounded-2xl bg-slate-50 p-4">
            <p className="text-xs uppercase text-slate-500">Domaines</p>
            <p className="text-2xl font-bold text-night">{domains.length}</p>
          </div>

          <div className="rounded-2xl bg-slate-50 p-4">
            <p className="text-xs uppercase text-slate-500">Projets</p>
            <p className="text-2xl font-bold text-night">{projects.length}</p>
          </div>

          <div className="rounded-2xl bg-slate-50 p-4">
            <p className="text-xs uppercase text-slate-500">Acteurs</p>
            <p className="text-2xl font-bold text-night">{actors.length}</p>
          </div>

          <div className="rounded-2xl bg-slate-50 p-4">
            <p className="text-xs uppercase text-slate-500">Opportunités</p>
            <p className="text-2xl font-bold text-night">{commercialOpportunities.length}</p>
          </div>

          <div className="rounded-2xl bg-slate-50 p-4">
            <p className="text-xs uppercase text-slate-500">Lignes source</p>
            <p className="text-2xl font-bold text-night">{sourceDataAudit.length}</p>
          </div>

          <div className="rounded-2xl bg-slate-50 p-4">
            <p className="text-xs uppercase text-slate-500">Pages PDF</p>
            <p className="text-2xl font-bold text-night">{sourceDocuments.reduce((total, document) => total + document.pageCount, 0)}</p>
          </div>
        </div>
      </div>

      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
        <h3 className="flex items-center gap-2 text-lg font-bold text-night">
          <Info className="h-5 w-5 text-orangeAccent" /> Notes de contexte commercial
        </h3>

        <ul className="mt-3 space-y-2 text-sm text-slate-700">
          {commercialContext.notes.map((note) => (
            <li key={note}>• {note}</li>
          ))}
        </ul>
      </div>
      ...
      <div className="rounded-3xl border border-orangeAccent/30 bg-orangeAccent/5 p-5 shadow-soft">
        <h3 className="flex items-center gap-2 text-lg font-bold text-night">
          <Database className="h-5 w-5 text-orangeAccent" /> Données source exhaustives et traçabilité
        </h3>
              ...
              {sourceDataAudit.map((item) => (
                <tr key={item.id} className="align-top hover:bg-orange-50/50">
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    {item.relatedActors.map((actorId) => getActorById(actorId)?.name ?? actorId).join(" · ") || "Non lié à un acteur précis"}
                  </td>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
        <h3 className="flex items-center gap-2 text-lg font-bold text-night">
          <FileJson className="h-5 w-5 text-cyanAccent" /> Texte source complet intégré dans l'application
        </h3>
              ...
              <div className="mt-3 grid gap-3">
                {document.pages.map((page) => (
                  <details key={`${document.id}-${page.page}`} className="rounded-xl border border-slate-200 bg-white p-3">
                    ...
                    <pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-950 p-4 text-xs leading-relaxed text-slate-100 scrollbar-thin">
                      {page.text}
                    </pre>
                  </details>
                ))}
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
        <h3 className="flex items-center gap-2 text-lg font-bold text-night">
          <Rows3 className="h-5 w-5 text-cyanAccent" /> Projets et relations
        </h3>
            ...
            <tbody className="divide-y divide-slate-100">
              {projects.map((project) => (
                <tr key={project.id} className="align-top hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    {project.actors.map((relation) => `${getActorById(relation.actorId)?.name ?? relation.actorId} (${relation.roleType})`).join(" · ")}
                  </td>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
        <h3 className="flex items-center gap-2 text-lg font-bold text-night">
          <FileJson className="h-5 w-5 text-orangeAccent" /> Acteurs
        </h3>
...
export default function App() {
  ...
  const mapTitle =
    filters.domains[0] ??
    (activeTab === "defense" ? "Défense / propulsion navale" : activeTab === "civil" ? "Nucléaire civil" : "Écosystème nucléaire");
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto grid max-w-[1800px] gap-4 px-4 py-4 lg:grid-cols-[320px_minmax(0,1fr)_390px] lg:px-6">
        ...
        <div className="space-y-4 lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-auto lg:pr-1 scrollbar-thin">
          {selectedActor ? (
            <ActorCard actor={selectedActor} onSelectProject={selectProject} />
          ) : (
            <ProjectCard project={selectedProject} onSelectActor={selectActor} />
          )}
        </div>
      </main>
    </div>
